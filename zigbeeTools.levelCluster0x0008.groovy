...
void setLevel(inputLevel, duration = 0 ) { setLevel(level:inputLevel, duration:duration) }
void setLevel(Map params = [:]) {
	Map inputs = [ep: getEndpointId(device) , level: 50 , duration: 0 ] << params
	...
	sendZCLAdvanced(
		...
		commandId: 0x04,
		commandPayload: [hexLevel, hexRate]
		)
	...
	setLastSentCommand(clusterId:"0008", commandId:0x04, commandData:thisCommandData, ep:(inputs.ep))
}
...
void processGlobalResponse0x0104_0008(Map descMap) {
	...
	switch(descMap.command) {
		...
		case "0B": // Default Response
			...
			Map whatWasSent = removeLastSentCommand(clusterId:"0008", commandId:lastCommand, ep:ep)
            if (logEnable) log.debug "Retrieved a copy of what was sent. Got: ${whatWasSent.inspect()}" 
			if ((status in ["00"]) && whatWasSent) {
...
void configure0x0104_0008(String ep = getEndpointId(device)) {
...
void unbind0x0104_0008(String ep = getEndpointId(device)) {
...
void initialize0x0104_0008(String ep = getEndpointId(device)) {
	// configure0x0104_0008(ep)
	refresh0x0104_0008(ep)
}

void refresh0x0104_0008(String ep = getEndpointId(device)){

   sendZCLAdvanced(
		clusterId: 0x0008 , 
		destinationEndpoint: ep, 
		commandId: 0x00,             // 00 = Read attributes, ZCL Table 2-3.
		commandPayload: ["0000", "0001", "0010", "0012", "0013"] // List of attributes of interest [0x0000, 0x0001, 0x0010, 0x0012, 0x0013] in reversed octet form
	)
}